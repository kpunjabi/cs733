...
var idToport[] int=make([]int,6)

func TestMain(m *testing.M) {	
	id:=1
	port:=8090
	
	var server [5]interface{}
	var args[] string=make([]string,3)
	name:="/home/kuldeep/Gowork1/bin/Assignment1Combined"
	args[1]=strconv.Itoa(id)
	...
	m.Run()
	for i:=0;i<5;i++{
 		 err:= server[i].(*os.Process).Signal(syscall.SIGKILL)
...
func mkClient(t *testing.T) *Client {
	var client *Client
	raddr, err := net.ResolveTCPAddr("tcp", "localhost:8090")
	if err == nil {
...
func ClientRedirected( id int) *Client {
	var client *Client
	raddr, err := net.ResolveTCPAddr("tcp", fmt.Sprintf("localhost:%d",idToport[id]))
	if err == nil {
		conn, err := net.DialTCP("tcp", nil, raddr)
		if err == nil {
			client = &Client{conn: conn, reader: bufio.NewReader(conn)}
...
func (cl *Client) rcv() (msg *Msg, err error) {
	...
	if err == nil {
		...
		if msg.Kind == 'C' {
			...
			cl.close()
			cl =ClientRedirected(msg.Version)
